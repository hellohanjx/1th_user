...
uint8_t IIC_Read_Byte(unsigned char ack)
{
  ...
  for(i=0;i<8;i++ )
    ...
    Delay_Us(2);
		IIC_SCL(1);
    receive<<=1;//这个是为了那个移位寄存器吗？？把数据顶出去？
    if(IIC_SDA_STATE) receive++; //读端口输入寄存器的值 
		Delay_Us(1); 
  }					 
...
{				  
	uint8_t temp=0;		  	    																 
	IIC_Start();  //根据 24lc512 时序图 ，转备发送地址 相当于i2c总线的 S
	if(EE_TYPE>AT24C16)
	{
		IIC_Send_Byte(0XA0);	                         //发送器件地址与写命令（0xA0最低位为0表示写，为1标志读）
		IIC_Wait_Ack();	//等待回应，ack 相当于i2c总线的A
