...
{
	g_lcd.background = BLUE;				//设置背景色
	g_lcd.foreground = WHITE;				//设置前景色
...
void  lcd_show_num(uint32_t num,uint16_t x,uint16_t y ,uint16_t foreground, uint16_t background, const BX_FONT font ,uint8_t type, uint8_t size) 
{
	...
	if(type != 0 && size < 11)
	{
		memcpy(&format[size-len], format, len + 1);
...
/*
*函数名称  show_money
*入口参数：money 需要打印的金额，x 起始行   y 起始列
*出口参数：无
*功能说明：在屏幕上打印金额的函数
*/
void  lcd_show_money(uint32_t money,uint16_t x,uint16_t y, uint16_t foreground, uint16_t background, const BX_FONT font)
{
	char format[12];//多了一位小数点，加上结束符最大12个字符
	char dis[11];
	uint8_t len,i;
	
	for(i = 0; i < g_vm.priceBit + 1 && i < 10; i++)//小数点占位也算进去
	dis[i] = ' ';
	dis[i] = 0;
	
	len = sprintf(format,"%u",money);
	if(g_vm.point != 0)
	{
	...
	format[len]='\0';
	lcd_show_string(dis,x,y, foreground, background, font);//刷干净要显示的区域
	lcd_show_string(format,x,y, foreground, background, font);
}
...
void lcd_init(void)
{
	...
	#if SCREEN == SCREENRA8875	//横屏
		bsp_lcd_set_directrion(0, &g_lcd.xSize, &g_lcd.ySize);		//屏幕方向设置
	#else	//竖屏
		bsp_lcd_set_directrion(1, &g_lcd.xSize, &g_lcd.ySize);		//屏幕方向设置
	#endif
	lcd_clear(); 
}
